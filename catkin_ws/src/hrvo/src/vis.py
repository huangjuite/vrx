...
def visualize_traj_dynamic(ws_model, X, U, goal, time = None, name=None, x_lim=[-1,6], y_lim=[-1,6]):
    figure = pyplot.figure(figsize=(10, 10),dpi=80)
    ...
    for i in range(0,len(X)):
        ...
        ax.add_patch(robot)
        #----------plot velocity
        ax.arrow(X[i][0], X[i][1], U[i][0], U[i][1], head_width=0.8, head_length=1.6, fc=cmap(i), ec=cmap(i))
        # ax.text(X[i][0]-0.1, X[i][1]-0.1, r'$%s$' %i, fontsize=15, fontweight = 'bold',zorder=3)
        # ax.text(X[i][0]-0.1, X[i][1]-0.1, r'$boat$', fontsize=15, fontweight = 'bold',zorder=3)
        ax.plot([goal[i][0]], [goal[i][1]], '*', color=cmap(i), markersize =15,linewidth=3.0)
    if time:
        ax.text(2,50,'$t=%.1f s$' %time,
